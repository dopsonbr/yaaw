...
- `format.sh`: format Swift source and `Package.swift` with the repo `.swift-format` configuration.
- `lint.sh`: lint Swift source and `Package.swift` with the repo `.swift-format` configuration.
...
Formatting and linting are script-backed and command-line first. Run `scripts/format.sh`
before opening a PR, and use `scripts/lint.sh` to match the GitHub Tests workflow.
