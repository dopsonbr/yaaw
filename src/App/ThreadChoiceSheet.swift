...
/// New-thread modal: agent card grid + a details panel (command / permissions /
/// extra args). Consumes `WorkspaceStore.createThread` and `SettingsStore` for
/// the per-agent launch defaults and permission presets.
struct ThreadChoiceSheet: View {
    ...
    init(workspace: WorkspaceStore, settings: SettingsStore, project: Project) {
        self.workspace = workspace
        self.settings = settings
        self.project = project
        let initialAgent = settings.defaultAgentCLI
        let initialOptions = settings.configuredLaunchOptions(for: initialAgent)
        _selectedAgentCLI = State(initialValue: initialAgent)
        _executableName = State(
            initialValue: initialOptions.executableName
                ?? settings.configuration.agentExecutableName(for: initialAgent)
        )
        ...
        LazyVGrid(
            ...
            ForEach(AgentCLIKind.allCases) { agentCLI in
                AgentStartCard(
                    ...
                    isDefault: settings.defaultAgentCLI == agentCLI,
    ...
    private var detailsPanel: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 8) {
                ...
                if selectedAgentCLI == settings.defaultAgentCLI {
                    Text("Default")
                        .font(fonts.interfaceFont(sizeOffset: -2, weight: .semibold))
    ...
    private var permissionModes: [AgentPermissionMode] {
        settings.permissionModes(for: selectedAgentCLI)
    }
    ...
    private func selectAgent(_ agentCLI: AgentCLIKind) {
        ...
        let options = settings.configuredLaunchOptions(for: agentCLI)
        executableName =
            options.executableName ?? settings.configuration.agentExecutableName(for: agentCLI)
    ...
    private func commandSummary(for agentCLI: AgentCLIKind) -> String {
        settings.configuredLaunchOptions(for: agentCLI).executableName
            ?? settings.configuration.agentExecutableName(for: agentCLI)
    }
    ...
    private func permissionSummary(for agentCLI: AgentCLIKind) -> String {
        ...
        return settings.permissionModes(for: agentCLI).first { $0.id == modeID }?.displayName
            ?? modeID
    }

    private func argumentsSummary(for agentCLI: AgentCLIKind) -> String {
        let arguments = settings.configuredLaunchOptions(for: agentCLI).additionalArguments
        ...
        do {
            ...
            let launchOptions = AgentLaunchOptions(
                executableName: command.isEmpty
                    ? settings.configuration.agentExecutableName(for: selectedAgentCLI)
                    : command,
