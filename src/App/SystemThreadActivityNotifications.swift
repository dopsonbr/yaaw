...
/// Posts thread-activity banners via Notification Center. The stores call this
/// through the `ThreadActivityNotificationDispatching` protocol so `YAAWKit`
/// never depends on `UserNotifications`/AppKit.
@MainActor
...
/// Reflects the unread-thread count onto the Dock tile badge.
@MainActor
