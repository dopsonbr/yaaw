...
final class TreeBuilderBenchmarks: BenchmarkCase {
    ...
    private lazy var directoryHeavyEntries150k: [FileBrowserEntry] =
        Self.synthesizeDirectoryHeavyEntries(directoryCount: 120_000, fileCount: 30_000)
    ...
    func test_bench_visibleRows_index_150k_oneExpandedBranch() throws {
        ...
        let expandedFolders: Set<String> = ["src", "src/core"]
        BenchmarkSupport.report(
            "visibleRows index 150k oneExpanded",
            BenchmarkSupport.median(iterations: 10) {
                _ = FileBrowserTreeBuilder.visibleRows(
    ...
    func test_bench_visibleRows_index_150k_directoryHeavy() throws {
        ...
        BenchmarkSupport.report(
            "visibleRows index 150k dirHeavy",
            BenchmarkSupport.median(iterations: 10) {
                _ = FileBrowserTreeBuilder.visibleRows(
                    childrenIndex: index, expandedFolders: expandedFolders, limit: 10_000)
            })
