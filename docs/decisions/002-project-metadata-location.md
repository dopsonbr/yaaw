...
1. **App-owned only.** All metadata in app SQLite + YAML settings. No files written to project directories. Survives `git clean`. Does not survive moving the project root unless the user re-adds it.
